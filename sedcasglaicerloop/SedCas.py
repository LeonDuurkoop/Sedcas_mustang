...
class SedCas:
    ...
    def __init__(self, climate_directory):
        self.cwd = os.getcwd()
        self.climate_directory = climate_directory
        self.ls = os.listdir(self.climate_directory)
        self.climatefile = None
        self.paramfile = None

    def load_climate(self, start_date=None, end_date=None):
    ...
    def run_hydro(self):
        ...
        for i in range(self.n_HRU):
            ...
            if self.HRUs[i] == 'glacier':
                g = mod.degree_day_model(self.Ta.copy(), self.Pr.copy(), 0.27, self.Tsa, self.Tsm, s0=10000000000000000000000, Asnow = self.Asnow[i], Asoil = self.Anosnow[i])
                glacier.append(g) 
    ...
    def save_output(self, climate_file_number):
        hydro_filename = f'Hydro.{climate_file_number}.out'
        sediment_filename = f'Sediment.{climate_file_number}.out'

        self.hydro.to_csv(hydro_filename, header=True)
        ...
        sedout.to_csv(sediment_filename, header=True)
